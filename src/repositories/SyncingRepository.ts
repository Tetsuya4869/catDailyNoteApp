...
export class SyncingRepository implements AppRepository {
  ...
  private async mirror(operation: () => Promise<void>) {
    try { await operation(); } catch (error) { console.warn('[sync] remote write deferred', error); }
  }

  async pullRemote(): Promise<void> {
    const [cats, entries, health, appointments] = await Promise.all([this.remote.getAllCats(), this.remote.getDiaryEntries(), this.remote.getHealthRecords(), this.remote.getAppointments()]);
  ...
  async pushLocal(): Promise<void> {
    const [cats, entries, health, appointments] = await Promise.all([this.local.getAllCats(), this.local.getDiaryEntries(), this.local.getHealthRecords(), this.local.getAppointments()]);
