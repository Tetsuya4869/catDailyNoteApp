...
export class SupabaseRepository implements AppRepository {
  ...
  private async request<T>(table: string, init: RequestInit = {}, query = ''): Promise<T> {
    const response = await fetch(`${this.session.url.replace(/\/$/, '')}/rest/v1/${table}${query}`, {
      ...init,
      headers: {
  ...
  private userFilter(extra = '') {
    return `?user_id=eq.${encodeURIComponent(this.session.userId)}${extra}`;
  }

  private catFromRow(row: Row): Cat {
    return { id: String(row.id), name: String(row.name), color: row.color as Cat['color'], gender: (row.gender ?? undefined) as Cat['gender'], birthDate: (row.birth_date ?? undefined) as string | undefined, weightGoal: row.weight_goal == null ? undefined : Number(row.weight_goal), photoUri: (row.photo_uri ?? undefined) as string | undefined, createdAt: String(row.created_at), archivedAt: (row.archived_at ?? undefined) as string | undefined };
  }
  ...
  async saveCat(cat: Cat) { await this.request('cats', { method: 'POST', body: JSON.stringify(this.catToRow(cat)) }, '?on_conflict=id'); }
  ...
  async saveDiaryEntry(entry: DiaryEntry) { await this.request('diary_entries', { method: 'POST', body: JSON.stringify(this.diaryToRow(entry)) }, '?on_conflict=id'); }
