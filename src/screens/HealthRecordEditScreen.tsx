...
export default function HealthRecordEditScreen({ navigation, route }: Props) {
  ...
  async function handleSave() {
    ...
    if (type === 'weight' && (!weight.trim() || !Number.isFinite(parsedWeight) || parsedWeight <= 0)) {
      Alert.alert('入力確認', '体重を正しく入力してください');
      return;
    }
    const weightKg = type === 'weight' ? parsedWeight : undefined;
  ...
  return <View style={styles.container}>
    <ScrollView contentContainerStyle={styles.content}>
      ...
      <View style={styles.typeGrid}>{types.map((item) => <TouchableOpacity key={item} style={[styles.typeButton, item === type && styles.active]} onPress={() => setType(item)}><Text style={styles.typeEmoji}>{healthTypeEmojis[item]}</Text><Text style={[styles.typeText, item === type && styles.activeText]}>{healthTypeLabels[item]}</Text></TouchableOpacity>)}</View>
