...
import { Cat, CatColor, CatGender, catColorEmojis, catColorLabels, catGenderSymbols } from '../types';
...
type Props = { navigation: NativeStackNavigationProp<RootStackParamList, 'CatEdit'>; route: RouteProp<RootStackParamList, 'CatEdit'> };
...
export default function CatEditScreen({ navigation, route }: Props) {
  ...
  useEffect(() => {
    if (!editId) return;
    getCatById(editId).then((cat) => {
      if (!cat) return;
      setName(cat.name); setColor(cat.color); setGender(cat.gender ?? 'unknown'); setBirthDate(cat.birthDate); setPhotoUri(cat.photoUri); setCreatedAt(cat.createdAt);
    });
  }, [editId]);
  ...
  async function handleSave() {
    ...
    const cat: Cat = { id: editId || Date.now().toString(), name: name.trim(), color, gender, birthDate, photoUri, createdAt: createdAt ?? new Date().toISOString() };
    await saveCat(cat);
    await refreshCats();
...
const createStyles = (colors: ThemeColors) => StyleSheet.create({
  ...
  photoButton: { alignSelf: 'center', marginBottom: spacing.xl, borderRadius: 75, overflow: 'hidden' },
  photo: { width: 150, height: 150, borderRadius: 75 },
  photoPlaceholder: { width: 150, height: 150, borderRadius: 75, backgroundColor: colors.backgroundMuted, borderWidth: 2, borderStyle: 'dashed', borderColor: colors.border, alignItems: 'center', justifyContent: 'center' },
  ...
  choice: { minWidth: 74, alignItems: 'center', backgroundColor: colors.card, borderRadius: borderRadius.md, padding: spacing.sm },
  choiceEmoji: { fontSize: 24 },
  choiceText: { color: colors.textSecondary, fontSize: 10, marginTop: 4 },
  gender: { flex: 1, alignItems: 'center', backgroundColor: colors.card, borderRadius: borderRadius.md, padding: spacing.md },
  genderText: { color: colors.textSecondary },
  active: { backgroundColor: colors.primary },
  activeText: { color: '#fff', fontWeight: 'bold' },
  dateText: { color: colors.text, fontSize: 16 },
  clear: { color: colors.primary, marginTop: -spacing.lg, marginBottom: spacing.xl },
  save: { backgroundColor: colors.primary, margin: spacing.xl, padding: spacing.lg, borderRadius: borderRadius.md, alignItems: 'center' },
