...
export default function DiaryEntryScreen({ navigation, route }: Props) {
  ...
  const { photoUri, loadOriginal, selectPickedUri, clearSelection, commit: commitPhoto, deleteAll: deletePhotos } = useManagedPhoto();

  useEffect(() => {
    if (!editId) return;
    repository.getDiaryEntryById(editId).then((entry) => {
      if (!entry) return;
  ...
  function removePhoto() {
    Alert.alert('写真を削除', '保存すると写真を削除します。よろしいですか？', [
      { text: 'キャンセル', style: 'cancel' },
      { text: '削除', style: 'destructive', onPress: clearSelection },
    ]);
  }
  ...
  function handleDelete() {
    ...
    Alert.alert('削除確認', 'この日記を削除しますか？', [
      { text: 'キャンセル', style: 'cancel' },
      { text: '削除', style: 'destructive', onPress: async () => { await repository.deleteDiaryEntry(editId); await deletePhotos(); navigation.goBack(); } },
    ]);
  }
