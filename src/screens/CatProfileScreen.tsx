...
export default function CatProfileScreen() {
  ...
  useFocusEffect(useCallback(() => {
    ...
    Promise.all([
      ...
      repository.getHealthRecordsByCat(catId),
      repository.getWeightSeries(catId),
      repository.getAppointmentsByCat(catId),
    ]).then(([all, healthRecords, weightSeries, appointmentItems]) => {
      if (!active) return;
  ...
  if (!cat) {
    return <View style={styles.missing}><Text style={styles.missingText}>猫が見つかりませんでした</Text></View>;
  }
  ...
  async function toggleAppointment(item: Appointment) {
    await repository.saveAppointment({ ...item, done: !item.done });
    ...
    return <>
      <View style={styles.header}>
        ...
        <View style={styles.avatarWrap}>
          {cat.photoUri ? <Image source={{ uri: cat.photoUri }} style={styles.avatar} /> : <View style={[styles.avatar, styles.avatarPlaceholder]}><Text style={styles.avatarEmoji}>{catColorEmojis[cat.color]}</Text></View>}
        </View>
      ...
      <View style={styles.segmentControl}>
        {(['diary', 'health', 'album'] as Segment[]).map((key) => <TouchableOpacity key={key} style={[styles.segment, segment === key && styles.segmentActive]} onPress={() => setSegment(key)}><Text style={[styles.segmentText, segment === key && styles.segmentTextActive]}>{key === 'diary' ? '📖 日記' : key === 'health' ? '🏥 健康' : '🖼 アルバム'}</Text></TouchableOpacity>)}
      </View>
    </>;
  }
  ...
  function HealthSection() {
    ...
    return <View style={styles.section}>
      <View style={styles.actionRow}>
        <TouchableOpacity style={styles.action} onPress={() => navigation.navigate('HealthRecordEdit', { catId })}><Text style={styles.actionText}>＋ 健康記録</Text></TouchableOpacity>
        <TouchableOpacity style={styles.actionSecondary} onPress={() => navigation.navigate('AppointmentEdit', { catId })}><Text style={styles.actionSecondaryText}>＋ 予定</Text></TouchableOpacity>
      </View>
...
function Empty({ styles, emoji, text }: { styles: ReturnType<typeof createStyles>; emoji: string; text: string }) {
  return <View style={styles.empty}><Text style={styles.emptyEmoji}>{emoji}</Text><Text style={styles.missingText}>{text}</Text></View>;
}
